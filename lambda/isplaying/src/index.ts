...
exports.handler = async (event: APIGatewayEvent) => {
    ...
    try {
        ...
        if (body.uid && typeof body['isPlaying'] === "boolean") {
            ...
            await Promise.all(roomsForJump.Item.users.map(async (user: string) => await sendMessage(user, JSON.stringify({ action: 'isPlaying', 'isPlaying': body['isPlaying'] }))));
        } else {
            await sendMessage(user_id, JSON.stringify({ status: 400, message: 'uid or isPlaying is missing' }));
        }
    } catch (error) {
